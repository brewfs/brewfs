...
mod op {
    ...
    impl Init<'_> {
        ...
        pub(crate) fn reply(&self, config: &crate::KernelConfig) -> ResponseData {
            ...
            let init = if cfg!(all(feature = "fuse-t", target_os = "macos")) {
                // FUSE-T's NFS backend identifies the client profile from the
                // INIT reply size: a 24-byte (libfuse2-compatible) reply makes
                // it negotiate profile v2, which expects the macFUSE extended
                // attribute layout that fuser uses on macOS. Sending the full
                // 64-byte reply makes it choose profile v3, which expects the
                // standard 88-byte `fuse_attr` and breaks OPEN/READ.
                &init[..FUSE_COMPAT_22_INIT_OUT_SIZE]
            } else if self.arg.minor < 5 {
