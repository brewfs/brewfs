#!/usr/bin/env bash
# Generic local CI gate for the native-base worktree. Keep this aligned with
# AGENTS.md and record every command in the wrapper log for exact-SHA evidence.
set -u
set -o pipefail
...
scratch_dir=$(mktemp -d /tmp/brewfs-ci-gate.XXXXXX) || exit 1
