...
当前验收进度（2026-09-19）：173 项矩阵中 **143 PASS**、30
...
COW 页复用与摘要扫描/重写计数已由 PR07P 收口（FROZEN-005/FROZEN-006/FROZEN-008）。Frozen 索引三缝已由 PR07Q 收口（IDX-003/IDX-004/IDX-005）：内联 attrs 搬进外部 ValueRef 必须逐字节等同 canonical bytes（resolved 后 revision digest 不变，非 canonical/缩短/异对象一律拒绝）、repack 后稀疏 ordinal 只剪除本 Pack 死映射而不是重解释旧 slot（pinned 保留、外 Pack 行不删不认领）、readdir cookie 锚定最后返回条目因此 spool 驱逐后可重放且 cookie 表满时先失败不发坏 cookie。
