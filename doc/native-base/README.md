...
| 08C | randrw/metadata 匹配对照与 FUSE teardown（REGRESS-004/REGRESS-006）：三次设置完全匹配的 compose 对照（control/control2/candidate，64m/20s，fio-randwrite+fio-randrw+metaperf）全部 3/3 工具 pass、0 warning，candidate 相对 control 的每项差值都落在 control 自身重跑的噪声带之内（最坏 −3.5% 对 −22.4% 噪声，randwrite +20.7%），无 cost shift（tail 与 effective wall bw 同向、对象放大持平）；teardown 后容器/volume/mount/连接/进程/D-state 全 0，且默认 feature 集编译不到 native 模块，`generic/075`/`generic/014`/`iogen01` 排除状态不变 | 完成（[pr08c-compose-perf-and-teardown.log](logs/pr08c-compose-perf-and-teardown.log)） |
...
当前验收进度（2026-09-19）：173 项矩阵中 **172 PASS** 与 1 项 `NOT_RUN`（REGRESS-005 的 buffered/direct/mmap 形态按该项要求记录为
...
randrw 与 metadata 的匹配对照及 FUSE teardown 已由 PR08C 收口（REGRESS-004/REGRESS-006/INV-13）：用 AGENTS.md 指定的 compose runner 在同一主机上跑三次设置完全匹配的挂载级对照（默认 feature 的 control 与 control2 量噪声带、加 `--features native-packed-base` 的 candidate），三次都 `perf.complete`、3/3 工具 pass、runner warning summary 全 0，writeback 债务（buffer_dirty/live_dirty/recent_pending_upload）最后采样回到 0；candidate 相对 control 的 randrw read_bw −3.5%、write_bw −3.0%、randwrite write_bw +20.7%、metaperf wall +14.2%，而 control 自身重跑的噪声带是 read_bw −22.4%、write_bw −21.3%、write_p999 +784.4%，所以每一项都解释为 run-to-run 抖动而不是回退，`write_effective_wall_bw` 与 active 带宽同向、`partial_tail_ratio`/`upload_byte_amp`/`put_ops_per_gib_written` 持平，没有把成本从 fio 运行期搬进 close/flush/drain；teardown 逐项复核为容器 0 / volume 0 / FUSE mount 0 / FUSE 连接 0 / brewfs 进程 0 / D-state 任务 0，另有静态证据表明默认（挂载）feature 集不含 `native-packed-base`、`src/native_base` 仅有的 2 处生产 spawn 都在挂载路径编译不到的 P3 planner 里，因此不可能新增后台 buffer 任务。范围说明（诚实）：AGENTS.md 的 `--writeback-throughput-profile` + `PERF_FIO_SIZE=512m` 严格画像在本机未跑完（WSL2 vhdx 撑爆宿主卷、fio 容器 exit 135、WSL VM 掉线，产物 `perf-run-1789800955-17525` 保留），因此这是单次匹配对，只用于证明“无回退/无 cost shift”，不构成有统计效力的性能结论，也未据此更新任何性能表。
