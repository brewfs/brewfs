...
#[tokio::test]
async fn rename_exchange_rejects_ancestor_in_both_directions() {
    let meta = test_meta().await;
    let root = meta.root_ino();
    let ancestor = meta.mkdir(root, "ancestor".into()).await.unwrap();
    let descendant = meta.mkdir(ancestor, "descendant".into()).await.unwrap();
    ...
    assert_eq!(
        meta.lookup(ancestor, "descendant").await.unwrap(),
        Some(descendant)
    );
