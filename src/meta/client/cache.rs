...
/// Cached slice list for one chunk index, together with an optional backend
/// version token and the last time the token was validated against the store.
///
/// `version` is `None` for backends without version tracking (database, etcd,
/// TiKV); such entries are served from the cache until explicitly invalidated.
/// `validated_at` throttles remote version checks on the hot read path.
#[derive(Clone)]
pub(crate) struct CachedSlices {
    pub(crate) version: Option<u64>,
    pub(crate) slices: Vec<SliceDesc>,
    pub(crate) validated_at: Option<Instant>,
}
...
pub(crate) struct InodeEntry {
    ...
    /// Cache slice metadata per chunk index, with backend version token.
...
impl InodeCache {
    ...
    /// Return the cached slices together with their version token and last
    /// validation time, if present.
    ...
    /// Mark the cached slice entry for `chunk_index` as just validated against
    /// the backend. No-op when the entry was evicted in the meantime.
    pub(crate) async fn touch_slices_validation(&self, inode: i64, chunk_index: u64) {
        if let Some(node) = self.ttl_manager.get(&inode).await
            && let Some(mut cached) = node.slices.get_mut(&chunk_index)
        {
            cached.validated_at = Some(Instant::now());
        }
    }
