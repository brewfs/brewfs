...
async fn stage_distributed_tests(vm: &mut Machine, repo_root: &Path) -> Result<()> {
    let dist_tests = repo_root.join("project/brewfs/tests/scripts/distributed-tests");
...
fn render_cluster_env(
    ...
    let tests_root = repo_root.join("project/brewfs/tests/scripts/distributed-tests");
...
async fn run_distributed_tests(
  ...
  ssh -o BatchMode=yes -o StrictHostKeyChecking=no -i /root/.ssh/id_ed25519 root@$node \
    ...
    let host_results_root =
        repo_root.join("project/brewfs/tests/scripts/distributed-tests/results");
    coordinator
