...
BrewFS 的 VFS 结构已经比较完整：有 inode runtime、handle registry、reader/writer、append locks、modified tracker、background tasks，并通过 rfuse3 暴露 FUSE。差距主要不在“有没有函数”，而在 POSIX 边界是否被完整定义、跨客户端是否一致、错误是否按系统调用语义传播。
...
BrewFS 使用 `rfuse3`，FUSE 层较薄，优势是复杂度低。JuiceFS mount 侧包含大量 mount options：
