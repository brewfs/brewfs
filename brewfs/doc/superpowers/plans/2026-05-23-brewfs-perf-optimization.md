...
**Files:**
- Modify: `brewfs/src/vfs/fs/mod.rs:345-355`
- Modify: `brewfs/src/vfs/cache/lru_cache.rs` (keep file, mark as dead)
- Modify: `brewfs/src/vfs/cache/read_cache.rs` (keep trait, mark as unused)
...
```bash
cd /mnt/rk8s/project/brewfs && rg "get_block|put_block|remove_block|ReadCache|LruReadCache" brewfs/src/ --no-filename
```
...
Edit `brewfs/src/vfs/fs/mod.rs`, remove lines ~345-355:
```rust
...
- [ ] **Step 5: Commit**
```bash
git add brewfs/src/vfs/fs/mod.rs brewfs/src/vfs/cache/
...
**Files:**
- Modify: `brewfs/src/meta/client/cache.rs:94-102` (InodeCache)
- Modify: `brewfs/src/meta/stores/redis/mod.rs:1151-1153` (node_cache TTL)
- Modify: `brewfs/src/meta/client/mod.rs:884-903` (cached_stat path)
- Modify: `brewfs/src/vfs/fs/mod.rs` (FUSE attr timeout)
...
Edit `brewfs/src/meta/stores/redis/mod.rs:1151-1153`:
```rust
// Before:
...
```bash
cd /mnt/rk8s/project/brewfs && rg -n "format.*\bi\b.*\{" brewfs/src/meta/stores/redis/mod.rs | head -20
```

- [ ] **Step 3: Extend FUSE kernel attribute cache timeout**
Edit `brewfs/src/vfs/fs/mod.rs` — find where FUSE `attr_timeout` is set (likely in the mount options or `getattr` response). Set to 1.0s (matching JuiceFS default):
```rust
...
Find the `open()` handler in `brewfs/src/vfs/fs/mod.rs` and ensure it populates the `InodeCache`:
```rust
...
**Files:**
- Create: `brewfs/src/chunk/cache_integrity.rs`
- Modify: `brewfs/src/chunk/cache.rs:362-381` (DiskStorage::load — add CRC32C verify)
- Modify: `brewfs/src/chunk/cache.rs:260-290` (DiskStorage::store — add CRC32C write)
- Modify: `brewfs/src/chunk/store.rs:478-481` (async cache population)
...
In `brewfs/src/chunk/cache_integrity.rs`:
```rust
use crc32c::crc32c;
...
In `brewfs/src/chunk/cache.rs`, modify `DiskStorage::store()` (~line 260):
```rust
...
In `brewfs/src/chunk/cache.rs`, modify `DiskStorage::load()` (~line 362):
```rust
...
In `brewfs/src/chunk/store.rs:478-481`, change synchronous cache insert to spawn background:
```rust
// Before:
...
- [ ] **Step 7: Commit**
```bash
git add brewfs/Cargo.toml brewfs/src/chunk/
...
**Files:**
- Modify: `brewfs/src/chunk/store.rs:389-446` (small-range read path)
...
```bash
cd /mnt/rk8s/project/brewfs && rg "compression" brewfs/src/chunk/store.rs | head -10
```
...
In `brewfs/src/chunk/store.rs:410-419`, after `get_object_range`:
```rust
let read_len = client
...
- [ ] **Step 5: Commit**
```bash
git add brewfs/src/chunk/store.rs
...
**Files:**
- Modify: `brewfs/src/chunk/store.rs:147` (add page-level SingleFlight)
- Modify: `brewfs/src/chunk/store.rs:389-446` (page read path)
...
**Files:**
- Modify: `brewfs/src/chunk/store.rs:440-446` (after page cache insert, trigger prefetch)
- Modify: `brewfs/src/vfs/cache/prefetch.rs:56-149` (ensure prefetcher can handle block keys)
...
**Files:**
- Modify: `brewfs/src/vfs/cache/prefetch.rs:143`
...
**Files:**
- Modify: `brewfs/src/vfs/io/writer.rs:266-270` (has_idle_block)
- Modify: `brewfs/src/vfs/io/writer.rs:1485-1526` (spawn_upload_task)
...
**Files:**
- Modify: `brewfs/src/vfs/io/reader.rs:693-703` (prepare_ahead_slices)
...
**Files:**
- Modify: `brewfs/src/vfs/io/writer.rs:579-640` (try_commit)
- Modify: `brewfs/src/chunk/compact/worker.rs:177-274`
...
**Files:**
- Modify: `brewfs/src/vfs/io/writer.rs:1733-1764` (commit_chunk)
...
**Files:**
- Modify: `brewfs/src/vfs/cache/write_back.rs:60-212`
- Modify: `brewfs/src/vfs/cache/config.rs`
