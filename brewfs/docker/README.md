...
- **xfstests-prebuilt**：Dockerfile 需要 `brewfs/tests/scripts/xfstests-prebuilt/xfstests-prebuilt.tar.gz`，如果缺失需要先 `git lfs pull`。
...
- 拉取以下资源：
  - `project/brewfs/tests/scripts/xfstests-prebuilt/*.tar.gz`
  - `project/brewfs/tests/scripts/fuse3-bundle/fusermount3`
...
```text
project/brewfs/tests/scripts/xfstests_slayer.exclude
```
...
- `image: brewfs:local`
- `build.context: ../..`
- `build.dockerfile: brewfs/docker/Dockerfile`
